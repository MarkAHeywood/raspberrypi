FILE="opencv_source_install_x86.sh"
...
cmake -DCMAKE_BUILD_TYPE=RELEASE -DCMAKE_INSTALL_PREFIX=/usr/local -DINSTALL_PYTHON_EXAMPLES=ON -DINSTALL_C_EXAMPLES=ON -DBUILD_TESTS=OFF -DBUILD_EXAMPLES=ON -DOPENCV_EXTRA_MODULES_PATH=~/opencv_contrib-${VERSION}/modules ..
echo "---" >> ~/$CVLOG
DATE=$(date)
echo "--> $DATE" >> ~/$CVLOG
# -
